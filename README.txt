Instructions regarding the project setup, configuration and execution.
	...
	The latest version of Apache Spark (version 2.3.2) has been used for the development of the Movie Recommendation Engine.
	Python's microframework Flask () was used to support the RESTful API on top of the spark Engine.
	For the support of asynchronous Flask calculations, Celery task queue () has been used. The Celery client in Flask communicates to the Celery worker through Redis message broker.
...
Details of the implemented recommendation system (e.g., how the modules work and cooperate/communicate, where and how the data is being stored and processed, etc.)
		...
		- fishEngine.py
		- fishRecommenderApp.py
	fishEngine.py implements class recomSparkEngine which is responsible for all major functionalities of the Movie Recommendation System. During
	class initialization the following actions are performed:
	...
	Class method "trainALSModel" is responsible for finding the optimal latent variables (the dimension found during class initialization is utilized) that
